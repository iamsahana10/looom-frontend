        ...
        {!loading && !error && hasResults && (
          <Tabs defaultValue="account" className="w-full">
            <TabsList className="w-full rounded-none border-b border-black/20 bg-transparent h-11 p-0 gap-0">
              <TabsTrigger
                value="users"
                className="h-4 flex-1 rounded-none tect-[13px] font-medium text-gray-400 bg-transparant shadow-none data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:text-gray-900 data-[state=active]:font-semibold relative after:absolute after:botton-0 after:left-0 after:right-0 after:h-[2px] after:bg-gray-900 after:scale-x-0 data-[state=active]:after:scale-x-100 after:transparent-transfer after:duration-200"
              >
                People
            ...
            <TabsContent value="users" className="mt-0">
              {users.length === 0 ? (
                ...
                users.map((user) => (
                  <Link
                    to={`/people/${users.username}`}
                    className="flex item-center gap-3 px-5 py-3.5 hover:bg-gray-50 transition-colors border-b border-black/20 last:border-b-0"
                  >
                    <div className="w-10 h-10 rounded-full pb-gradient-to-br from-gray-700 to-gray-500 flex items-center justify-center text-white font-bold text-sm shrink-0">
